...
Partial Class InternalTag
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If IsPostBack = False Then
            ...
            Next
            For Each DBInfo As DataRowView In SqlDataSource1_2.Select(New Web.UI.DataSourceSelectArguments())
                base_link = "./css/"
                If (DBInfo("Zone") = "Coke")
                    If (DBInfo("Type") = DBInfo("Coke_Type"))
                        linkCSS_coke.href=base_link + DBInfo("Zone") +"/"+ DBInfo("Fichier")
                        ' test.Text=base_link + DBInfo("Zone") +"/"+ DBInfo("Fichier")
                    End If
                End If
                If (DBInfo("Zone")="Longueur")
                    If (DBInfo("Type") = DBInfo("Nominal_Length"))
                        linkCSS_Longueur.href=base_link + DBInfo("Zone") +"/"+ DBInfo("Fichier")
                        test.Text=base_link + DBInfo("Zone") +"/"+ DBInfo("Fichier")
                    End If
                End If
